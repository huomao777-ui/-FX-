...
func 消耗体力(val: float) -> void:
	...
	if not is_equal_approx(old, _current_value):
		print("StaminaSystem: 消耗体力 ", val, "，当前 ", _current_value)

## 恢复体力，val > 0
func 恢复体力(val: float) -> void:
	if val <= 0:
		return
	var old: float = _current_value
	_current_value = clampf(_current_value + val, _min_value, _max_value)
	_emit_value_changed()
	if not is_equal_approx(old, _current_value):
		print("StaminaSystem: 恢复体力 ", val, "，当前 ", _current_value)

## 提升体力上限（当前值同步增加），val > 0
func 提升上限(val: float) -> void:
	if val <= 0:
		return
	_max_value += val
	_current_value += val
	_emit_value_changed()
	print("StaminaSystem: 提升上限 ", val, "，新上限 ", _max_value)
