...
const DEFAULT_MIN: float = 1.0
const DEFAULT_MAX: float = 100.0
const DEFAULT_CURRENT: float = 50.0
...
func 降低精神(val: float) -> void:
	...
	if not is_equal_approx(old, _current_value):
		print("MentalSystem: 降低精神 ", val, "，当前 ", _current_value)

## 增加精神状态，val > 0
func 增加精神(val: float) -> void:
	if val <= 0:
		return
	var old: float = _current_value
	_current_value = clampf(_current_value + val, _min_value, _max_value)
	_emit_value_changed()
	if not is_equal_approx(old, _current_value):
		print("MentalSystem: 增加精神 ", val, "，当前 ", _current_value)
