...
const DEFAULT_MIN: float = 75.0
const DEFAULT_MAX: float = 300.0
const DEFAULT_CURRENT: float = 75.0
...
func 增加压力(val: float) -> void:
	...
	if not is_equal_approx(old, _current_value):
		print("StressSystem: 增加压力 ", val, "，当前 ", _current_value)

## 减少压力，val > 0
func 减少压力(val: float) -> void:
	if val <= 0:
		return
	var old: float = _current_value
	_current_value = clampf(_current_value - val, _min_value, _max_value)
	_emit_value_changed()
	if not is_equal_approx(old, _current_value):
		print("StressSystem: 减少压力 ", val, "，当前 ", _current_value)
