...
## 依赖: 同级 MarketEngine 提供 RMB 基准汇率
## 状态: 第一阶段
## 最后更新: 2026-06-12
...
## direction: TradePosition.Direction.LONG_FOREIGN 表示做多外币；LONG_RMB 表示做多人民币
func 开仓(货币代码: String, 方向: int, 手数: float, 杠杆: int, 平台: String = "国内") -> Dictionary:
	...
	var notional_rmb: float = normalized_lots * _get_lot_value_rmb()
	var margin_used: float = notional_rmb / float(max(杠杆, 1))
	var open_cost: float = _calculate_trade_cost(平台, notional_rmb)
...
func _close_position(position: TradePosition, reason: String, extra_slippage_rate: float) -> Dictionary:
	...
	var close_cost: float = _calculate_trade_cost(position.platform, position.notional_rmb, extra_slippage_rate)
...
func _calculate_trade_cost(platform: String, notional_rmb: float, extra_slippage_rate: float = 0.0) -> float:
	...
	return notional_rmb * (spread_rate + slippage_rate + max(extra_slippage_rate, 0.0))
...
func _get_lot_value_rmb() -> float:
	return float(_contract_rules.get("lot_value_rmb", 100000.0))
...
func _get_position_snapshot(position: TradePosition) -> Dictionary:
	...
	return {
		...
		"lots": position.lots,
		"notional_rmb": position.notional_rmb,
...
func _clamp_direction(direction: int) -> int:
	if direction == TradePosition.Direction.LONG_RMB:
		return TradePosition.Direction.LONG_RMB
