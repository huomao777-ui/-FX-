...
func _on_card_gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		if mouse_event.button_index == MOUSE_BUTTON_LEFT and mouse_event.pressed:
			_begin_drag(mouse_event.global_position.x)
			get_viewport().set_input_as_handled()
		return
	...
	if event is InputEventScreenTouch:
		...
		if touch_event.pressed:
			_begin_drag(touch_event.position.x)
			get_viewport().set_input_as_handled()
...
func _get_damped_offset(raw_offset_x: float) -> float:
	if _page_span <= 0.0:
		return raw_offset_x
	var left_limit: float = -_page_span
	var right_limit: float = _page_span
	var damped: float = clampf(raw_offset_x, left_limit, right_limit)
