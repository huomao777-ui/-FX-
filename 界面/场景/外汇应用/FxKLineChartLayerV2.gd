...
func _draw_candles(chart: Rect2, candles: Array, pa: Dictionary) -> void:
	...
	for i in range(candles.size()):
		...
		var hy: float = _p2y(h, pa, chart)
		var ly: float = _p2y(l, pa, chart)
		var oy: float = _p2y(o, pa, chart)
		var cy: float = _p2y(cl, pa, chart)
...
func _draw_price_axis_labels(chart: Rect2, pa: Dictionary, active: bool) -> void:
	...
	for i in range(labels.size()):
		...
		draw_string(font, Vector2(chart.end.x + 8.0, y), txt, HORIZONTAL_ALIGNMENT_LEFT, -1.0, 坐标字体大小, 坐标文字颜色)
...
func _draw_time_axis_labels(chart: Rect2, vol: Rect2, candles: Array, label_data: Array[Dictionary]) -> void:
	...
	for item in label_data:
		...
		draw_string(font, Vector2(x - 24.0, y), txt, HORIZONTAL_ALIGNMENT_CENTER, 78.0, 坐标字体大小, 坐标文字颜色)

func _get_time_label_data(candles: Array, vol: Rect2) -> Array[Dictionary]:
	var out: Array[Dictionary] = []
	var total: int = candles.size()
	if total > 0:
		var gap: float = vol.size.x / float(max(total + max(未来留白K线数, 0), 1))
		var step: int = _get_label_step()
		# 从最左侧K线（idx=0）开始向右每隔 step 根画一个标签，位置不随周期跳动
		var idx: int = 0
		while idx < total:
			var x: float = vol.position.x + gap * (float(idx) + 0.5)
			out.append({"x": x, "text": _get_time_axis_text(total - 1 - idx)})
			idx += step
	else:
		var n: int = max(下方刻度数量, 2)
		for i in range(n):
			var x: float = vol.position.x + vol.size.x * float(i) / float(n - 1)
			out.append({"x": x, "text": "--"})
	return out
...
func _draw_liquidation_line(chart: Rect2, pa: Dictionary, active: bool) -> void:
	...
	draw_string(font, Vector2(chart.end.x + 8.0, ly), txt, HORIZONTAL_ALIGNMENT_LEFT, -1.0, 坐标字体大小, 强平线颜色)

func _draw_axis_break_markers(chart: Rect2, pa: Dictionary, positions: Array) -> void:
	var side: String = str(pa.get("broken_side", ""))
	if side.is_empty() or positions.size() < 2:
		return
	# 断裂Y坐标：与_calc_broken_positions一致
	var pad: float = 10.0
...
func _p2y(price: float, pa: Dictionary, chart: Rect2) -> float:
	...
	return chart.end.y - chart.size.y * clampf(r, 0.0, 1.0)
...
func _p2ay(price: float, pa: Dictionary, chart: Rect2) -> float:
	...
	if side == "lower":
		var lower: float = float(labels[0])
		var normal_start: float = float(labels[2])  # 上区第一个刻度
		if price <= normal_start:
			# 下区：在line_positions[0]和line_positions[1]之间线性映射
			var r: float = clampf((price - lower) / max(float(labels[1]) - lower, 0.000001), 0.0, 1.0)
			return lerpf(float(line_positions[0]), float(line_positions[1]), r)
		# 上区：在line_positions[2]到line_positions[n-1]之间分段映射
		return _piecewise_p2y(price, labels, line_positions, 2, n - 1)
	if side == "upper":
		var last: int = n - 1
		var normal_end: float = float(labels[last - 2])  # 下区最后一个刻度
		var upper: float = float(labels[last])
		if price >= normal_end:
			# 上区：在line_positions[last-1]和line_positions[last]之间线性映射
			var r: float = clampf((price - normal_end) / max(upper - normal_end, 0.000001), 0.0, 1.0)
			return lerpf(float(line_positions[last - 1]), float(line_positions[last]), r)
		# 下区：在line_positions[0]到line_positions[last-2]之间分段映射
		return _piecewise_p2y(price, labels, line_positions, 0, last - 2)
	return _p2y(price, pa, chart)

## 为断裂情况计算正确的line_positions（下区/上区视觉压缩）
## Godot Y轴：顶部=小值，底部=大值
func _calc_broken_positions(pa: Dictionary, chart: Rect2) -> Array:
	var labels: Array = pa.get("labels", []) as Array
	var side: String = str(pa.get("broken_side", ""))
	var n: int = labels.size()
	var pad: float = 10.0
	var top: float = chart.position.y + pad   # 图表顶部Y（小值）
	var btm: float = chart.end.y - pad       # 图表底部Y（大值）
	var h: float = max(btm - top, 8.0)

	if side == "lower":
		# 下侧断裂：上区（原有刻度2..x-1）占顶部72%，下区（刻度x和x+1）占底部28%
		var upper_h: float = h * 0.70      # 上区高度
		var lower_h: float = h * 0.28      # 下区高度
		var break_y: float = btm - lower_h  # 断裂位置Y（下区顶部）
		var positions: Array = []
		# 下区2条：从下往上排
		positions.append(int(roundf(btm)))                  # 0=新刻度（最低，在底部）
		positions.append(int(roundf(break_y + lower_h * 0.45)))  # 1=原有最低刻度（下区中部）
		# 上区n-2条：从断裂处到顶部均匀排布
		var upper_count: int = n - 2
		for i in range(upper_count):
			var ratio: float = 0.0 if upper_count <= 1 else float(i) / float(upper_count - 1)
			# ratio=0 → 贴断裂处，ratio=1 → 贴顶部
			var y: float = break_y + upper_h * (1.0 - ratio)
			positions.append(int(roundf(y)))
		return positions

	if side == "upper":
		# 上侧断裂：下区（原有刻度0..x-2）占底部72%，上区（刻度x和x+1）占顶部28%
		var lower_h: float = h * 0.70
		var upper_h: float = h * 0.28
		var break_y: float = top + lower_h   # 断裂位置Y（上区底部）
		var positions: Array = []
		# 下区n-2条：从顶部到下区底部均匀排布
		var lower_count: int = n - 2
		for i in range(lower_count):
			var ratio: float = 0.0 if lower_count <= 1 else float(i) / float(lower_count - 1)
			var y: float = top + lower_h * ratio
			positions.append(int(roundf(y)))
		# 上区2条：从断裂处到顶部
		positions.append(int(roundf(break_y + upper_h * 0.45)))  # x-1=原有最高刻度
		positions.append(int(roundf(top + 4.0)))                  # x=新刻度（最高，在顶部）
		return positions
...
func _apply_liquidation_axis_break(data: Dictionary) -> Dictionary:
	...
	for lbl in labels:
		display.append(float(lbl))
	if _liquidation_line_price < mn:
		# 下侧断裂：新增一个刻度到强平线附近（不替换原刻度，避免贴合强平线）
		_append_break_tick(display, step_val, _liquidation_line_price, true)
		data["broken_side"] = "lower"
		data["break_from"] = _liquidation_line_price
		data["break_to"] = float(labels[0])
		data["visual_min"] = _liquidation_line_price
		data["visual_max"] = mx
	elif _liquidation_line_price > mx:
		_append_break_tick(display, step_val, _liquidation_line_price, false)
		data["broken_side"] = "upper"
		data["break_from"] = float(labels[labels.size() - 1])
		data["break_to"] = _liquidation_line_price
		data["visual_min"] = mn
		data["visual_max"] = _liquidation_line_price
	data["labels"] = display
	return data

## 断裂逻辑（x=右侧刻度数量）：
## 原有x个刻度的值完全不变，仅在显示层面插入断裂
## going_down: 在数组开头新增1个刻度（x+1），断裂在原有x-1和x之间
## going_up:   在数组末尾新增1个刻度（x+1），断裂在原有x-1和x之间
func _append_break_tick(display: Array, step: float, liq: float, going_down: bool) -> void:
	if going_down:
		# 新增x+1个刻度在强平线下方（按原步长计算）
		var new_val: float = snappedf(float(display[0]) - step, step * 0.0001)
		# 确保新刻度值 < 强平线 < 原有最低刻度
		if new_val >= liq:
			new_val = snappedf(liq - step * 0.5, step * 0.0001)
		if new_val >= float(display[0]):
			new_val = snappedf(float(display[0]) - step, step * 0.0001)
		display.insert(0, new_val)
	else:
		# 新增x+1个刻度在强平线上方（按原步长计算）
		var new_val: float = snappedf(float(display[-1]) + step, step * 0.0001)
		if new_val <= liq:
			new_val = snappedf(liq + step * 0.5, step * 0.0001)
		if new_val <= float(display[-1]):
			new_val = snappedf(float(display[-1]) + step, step * 0.0001)
		display.append(new_val)
...
func _apply_live_rate(rate: float) -> void:
	for tf in _get_supported_timeframes():
		...
		if bk != last_bk:
			...
			series.append({"open": prev_close, "high": max(prev_close, rate), "low": min(prev_close, rate), "close": rate, "volume": 1.0})
